\documentclass[tikz,border=10pt]{standalone}
\usepackage{tikz}
\usetikzlibrary{arrows.meta, positioning, calc}

\begin{document}
\begin{tikzpicture}[
  node/.style = {circle, draw, thick, minimum size=18mm, align=center},
  gazebo_node/.style = {circle, draw=red, thick, fill=red!10, minimum size=18mm, align=center},
  topic/.style = {draw, thick, rounded corners, minimum height=7mm, minimum width=20mm, align=center, inner sep=2pt},
  arrow/.style = {thick, -{Latex[width=2mm]}},
  node distance=3.2cm and 2.2cm
]

%--------------------------------------------------
% ROS nodes (circles)
%--------------------------------------------------
\node[node] (waypoint) {Waypoint\\Generator};
\node[node, right=4.0cm of waypoint] (controller) {Velocity\\Controller};
\node[gazebo_node, right=4.0cm of controller] (gazebo) {Gazebo\\(TurtleBot)};
\node[node, below=2.4cm of gazebo] (observer) {EKF\\(Observer)};

%--------------------------------------------------
% Topics (blocks)
%--------------------------------------------------
\node[topic] (t_reference) at ($(waypoint)!0.5!(controller)$)
  {\texttt{/reference}};

\node[topic] (t_cmd_vel) at ($(controller)!0.5!(gazebo)$)
  {\texttt{/cmd\_vel}};

\node[topic] (t_odom) at ($(gazebo)!0.5!(observer)$)
  {\texttt{/odom}};

% feedback topic
\node[topic, left=2.3cm of observer.west] (t_state_est)
  {\texttt{/state\_est}};

%--------------------------------------------------
% Connections
%--------------------------------------------------

% reference
\draw[arrow] (waypoint.east) -- (t_reference.west);
\draw[arrow] (t_reference.east) -- (controller.west);

% cmd_vel
\draw[arrow] (controller.east) -- (t_cmd_vel.west);
\draw[arrow] (t_cmd_vel.east) -- (gazebo.west);

% cmd_vel branch to Observer (south -> northwest)
\draw[arrow]
  (t_cmd_vel.south)
  |- (observer.north west);

% odom
\draw[arrow] (gazebo.south) -- (t_odom.north);
\draw[arrow] (t_odom.south) -- (observer.north);

% state_est feedback
\draw[arrow] (observer.west) -- (t_state_est.east);
\draw[arrow] (t_state_est.west) -| (controller.south);

\end{tikzpicture}
\end{document}
