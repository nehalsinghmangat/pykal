...
class EKF:
    ...
    def predict(
        ...
        *,
        ...
        state_names = self.sys._resolve_override_str_list(
            override_system_state_names, self.sys.state_names
        )

        input_names = self.sys._resolve_override_str_list(
        ...
        u = self.sys._resolve_override_func(
        ...
        uk = self.sys.smart_call(u, t=tk, expected_shape=(p_inputs, 1))
        ...
        if self.sys.system_type in ("cti", "ctv"):
            xdot = self.sys.smart_call(f, x=xk, u=uk, t=tk)
            x_pred = xk + xdot * dt

            if override_predict is False:
                P_pred = Pk + (Fk @ Pk + Pk @ Fk.T + Qk) * dt
            else:
                P_pred = self.sys.smart_call(override_predict)

        elif self.sys.system_type in ("dti", "dtv"):
            x_pred = self.sys.smart_call(f, x=xk, u=uk, t=tk)
    ...
    def update(
        ...
        *,
        ...
        measurement_names = self.sys._resolve_override_str_list(
        ...
        uk = self.sys.smart_call(u, t=tk, expected_shape=(p_inputs, 1))
        Hk = self.sys.smart_call(H, x=xk, u=uk, t=tk, expected_shape=(m_meas, n_states))
        Rk = self.sys.smart_call(R, x=xk, u=uk, t=tk, expected_shape=(m_meas, m_meas))
        y_pred = self.sys.smart_call(h, x=xk, u=uk, t=tk)
        ...
        if override_update is False:
            ...
            x_upd, P_upd = self.sys.smart_call(
                override_update,
                xk=xk,
                uk=uk,
                tk=tk,
                ...
                check_func_sign=False,
                check_func_output_type=False,
            )
