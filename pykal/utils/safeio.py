...
class SafeIO:
    ...
    def _validate_string_sequence(self, names: Any) -> List:
        """
    ...
    def aliases_for_x(self, candidate_aliases):
        self._aliases_for_x = self._validate_string_sequence(candidate_aliases)

    @aliases_for_u.setter
    def aliases_for_u(self, candidate_aliases):
        self._aliases_for_u = self._validate_string_sequence(candidate_aliases)

    @aliases_for_t.setter
    def aliases_for_t(self, candidate_aliases):
        self._aliases_for_t = self._validate_string_sequence(candidate_aliases)

    @staticmethod
    def _is_ndarray_hint(hint: Any) -> bool:
        """
        Check whether a type hint corresponds to a NumPy ndarray.
        ...
        -------
        bool
            True if the hint corresponds to NDArray or np.ndarray, False otherwise.
        """
        origin = get_origin(hint)
        return hint is NDArray or origin is NDArray or origin is np.ndarray

    @staticmethod
    def _is_optional_ndarray_hint(hint: Any) -> bool:
        ...
        return (
            origin is Union
            and any(SafeIO._is_ndarray_hint(arg) for arg in args)
            and type(None) in args
        )

    @staticmethod
    def _is_float_hint(hint: Any) -> bool:
        """
    ...
    def _validate_func_signature(self, func: Callable) -> Callable:
        ...
        for name, param in sig.parameters.items():
            if name not in expected_aliases:
                ...
                if param.default is inspect.Parameter.empty:
                    raise TypeError(
                        f"In `{func.__name__}`, unexpected parameter `{name}` must have a default value."
                    )
                continue  # Don't enforce annotation/type on extra kwargs
            ...
            if name in alias_x:
                if not SafeIO._is_ndarray_hint(hint):
                    raise TypeError(
            ...
            if name in alias_u:
                if not (
                    ...
                    or SafeIO._is_optional_ndarray_hint(hint)
                ):
                    raise TypeError(
            ...
            if name in alias_t:
                if not SafeIO._is_float_hint(hint):
                    raise TypeError(
        ...
        if "return" not in hints:
            raise TypeError(
                f"In `{func.__name__}`, return type annotation is missing. Must be NDArray[...]"
            )

        if not SafeIO._is_ndarray_hint(hints["return"]):
            raise TypeError(
                f"In `{func.__name__}`, return type must be NDArray[...] or compatible type, "
    ...
    def smart_call(
        ...
        **extra_kwargs: Any,
        ...
        for name in sig.parameters:
            if name in self.aliases_for_x and x is not None:
                call_kwargs[name] = x
            elif name in self.aliases_for_u and u is not None:
                call_kwargs[name] = u
            elif name in self.aliases_for_t and t is not None:
                call_kwargs[name] = t
